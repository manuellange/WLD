...
class ROCValidationCallback(tp.Callback):
    ...
    def _setup_graph(self):
        input_names = ['input', 'heights', 'left', 'label', 'wavelets', 'lbd.descriptor']
        ...
        self.saver = tf.train.Saver(max_to_keep=10, write_version=tf.train.SaverDef.V2, save_relative_paths=True)
    ...
    def _trigger_epoch(self):
        ...
        for batch_n in range(config.batch_num):
            batch = next(data)
            descs = self.validation_predictor(batch[0], batch[1], batch[2], batch[3], batch[4], batch[5])[0]

            # calculate lbd distance matrix
            lbd_dists = pairwise_lbd_distance(batch[5])
...
class Model(tp.ModelDesc):
    ...
    def get_logits(self, images, nfeatures):
        """From tensorpack_examples/imagenet-resnet.py"""
        with tp.argscope([tp.Conv2D, tp.MaxPooling,
                          tp.GlobalAvgPooling, tp.BatchNorm],
                         data_format="NHWC"):
            return resnet_backbone(
                images, self.num_blocks,
                preresnet_group if self.mode == 'preact' else resnet_group,
                self.block_func, nfeatures, bn=not config.disable_bn)

    def embed(self, x, heights, nfeatures=16, fixed_length = False,
              l_change_prob=0.25, l_change_delta=10):
        ...
        def get_n_logits(in1, in2, h):
            ...
            # in1 = tf.Print(in1, [tf.shape(in1), in1], "in11:", summarize=100000)
            # in1 = tf.image.adjust_brightness(in1, prob * delta)

            zero_alpha = tf.multiply(in1, [1., 1., 1., 0.])
            zero_alpha = tf.clip_by_value(zero_alpha, 0.0, 1.0)
            # zero_alpha = tf.Print(zero_alpha, [tf.shape(zero_alpha), zero_alpha], "zero_alpha:", summarize=100000)

            # scale values from [0; 1] to [-1; 1]
            # el = el * 2 - 1
            zero_alpha = zero_alpha * 2 - 1
            in2 = in2 * 2 - 1

            # el = tf.expand_dims(el, 0)
            zero_alpha = tf.expand_dims(zero_alpha, 0)
            in2 = tf.expand_dims(in2, 0)
            # el = tf.image.crop_to_bounding_box(el, 0, 0, h, config.C_WIDTH)

            # return self.get_logits(el, nfeatures)
            return self.get_logits(array([zero_alpha, in2]), nfeatures)
        ...
        if config.use_dense_net:
            ...
            if not fixed_length:
                ...
                embeddings = tf.map_fn(lambda el: get_n_logits(el[0], el[1], el[2]),
                                       (x[0], x[1], heights), dtype=tf.float32)
                ...
                x[0] = x[0] * 2 - 1
                x[1] = x[1] * 2 - 1
                # x[0] = tf.Print(x[0], [tf.shape(x[0]), x[0]], "xFix:", summarize=100000)
    ...
    def inputs(self):
        ...
        inputs += [
            ...
            tf.placeholder(tf.int32,
                           ...
                           "label"),
            tf.placeholder(tf.float32,
                           (None, config.C_HEIGHT, config.C_WIDTH, 8),
                           "wavelets"),
    ...
    def build_graph(self, *inputs):
        ...
        if (config.debug_cutout):
            em, heights, in1, img_sides, labels, wavelets, desc = inputs
        else:
            em, heights, img_sides, labels, wavelets, desc = inputs

        embeddings = self.embed([em, wavelets], heights, 16, config.fixed_length,
    ...
    def optimizer(self):
        lr = tf.get_variable('learning_rate', initializer=1e-4,  # 0.0,
                             trainable=False)
...
def get_config(ds, model, valid_ds=None):
    ...
    # setup callbacks
    if config.const_lr is None:
        lrcb = tp.ScheduledHyperParamSetter(
            "learning_rate",
            # TODO test this
            # [(0, 1e-3), (10, 1e-4), (20, 1e-7), (30, 1e-9)]),
            # [(0, 1e-3), (10, 1e-4), (20, 1e-7), (30, 1e-9), (40, 1e-11)]),
            [(0, 0.5), (1, 0.3), (2, 0.2), (3, 1e-1), (4, 0.05),
             (5, 0.02), (6, 1e-2), (7, 0.005), (8, 0.001), (9, 0.0005),
             (10, 0.0001), (11, 0.00008), (12, 0.00006), (13, 0.00004), (14, 0.00002),
             (15, 0.00001), (16, 1e-5), (17, 1e-5), (18, 1e-5), (19, 1e-8),
             (20, 1e-7), (21, 1e-6), (22, 1e-5), (23, 1e-5), (24, 1e-5),
             (25, 1e-5), (26, 1e-5), (27, 1e-5), (28, 1e-5), (29, 1e-5)])
    else:
        lrcb = tp.ScheduledHyperParamSetter("learning_rate", [(0, config.const_lr)])
    callbacks = [
            # tp.ModelSaver(),
            tp.PeriodicTrigger(tp.ModelSaver(max_to_keep=50),
                               ...
                               every_k_epochs=1),
            lrcb,
            tp.HumanHyperParamSetter('learning_rate', file_name='hyper.txt'),
            AbortCallback()
        ]
